db={
    ...
    "vuz24":[
        {
            "q": "1. Тамга взималась с ввозимых товаров в следующем объеме:",
            "a": ["в размере 15% от стоимости товара", "в зависимости от количества товаров", "взималась по ставке «от рубля алтын»", "в размере от 3 до 10% стоимости товара"]
        },
        {
            "q": "2. Явка представляла собой:",
            "a": ["пошлину за предъявленный в таможне товар по прибытию в город", "платеж за приезд купца в город", "пошлина с привозных товаров, которая взималась с оценки товаров, помещаемых в гостином дворе", "пошлина за провоз товара"]
        },
        {
            "q": "3. Международный день таможенника отмечается в:",
            "a": ["октябре", "январе", "мае", "сентябре"]
        },
        {
            "q": "4. Глава «О мытах и о перевозах, и о мостах», регулирующая таможенное дело во второй половине XVII века, была в:",
            "a": ["Новоторговом уставе 1667 года", "Указе 1653 г «О взимании таможенной пошлины с товаров в Москве и городах с показанием поскольку взято и с каких товаров»", "Таможенном кодексе 1650 года", "Соборном уложении 1649 года"]
        },
        {
            "q": "5. Когда был принят Торговый устав - закон, регламентировавший нормы русского торгового права и вводивший единую торговую пошлину?",
            "a": ["1653 год", "1657 год", "1663 год", "1691 год"]
        },
        {
            "q": "6. Запрещена пересылка в международных почтовых отправлениях:",
            "a": ["игрушек", "рекламной продукции", "природных алмазов", "звукозаписывающей аппаратуры"]
        },
        {
            "q": "7. Физическое лицо должно выбрать зеленый коридор, если в его сопровождаемом багаже:",
            "a": ["Отсутствуют товары, подлежащие таможенному декларированию", "Имеются товары, подлежащие таможенному декларированию", "Имеются товары, в отношении которых подлежат уплате таможенные пошлины, налоги", "Зеленый коридор выбирается по желанию"]
        },
        {
            "q": "8. Товары для личного пользования не могут быть выпущены:",
            "a": ["Для внутреннего потребления", "В свободное обращение", "Для вывоза", "Для временного нахождения на таможенной территории ЕАЭС"]
        },
        {
            "q": "9. Согласно ТК ЕАЭС, таможенный контроль может проводиться:",
            "a": ["Налоговыми органами", "Таможенными органами", "Погранслужбой", "Ространснадзором"]
        },
        {
            "q": "10. К транспортным средствам международной перевозки не относится:",
            "a": ["Автотранспорт", "Ж/Д транспорт", "Водный транспорт", "Трубопроводный транспорт"]
        },
        {
            "q": "11. Для декларирования транспортного средства международной перевозки предназначена:",
            "a": ["Декларация на товары", "Транзитная декларация", "Декларация на транспортное средство", "Пассажирская декларация"]
        },
        {
            "q": "12. Под таможенную процедуру таможенного транзита не могут помещаться:",
            "a": ["товары, перемещаемые по линиям электропередачи", "товары, перемещаемые трубопроводом", "товары личного пользования", "припасы"]
        },
        {
            "q": "13. Если таможенный орган требует вскрыть консульскую вализу, а уполномоченное лицо иностранного государства отказывается это делать, то:",
            "a": ["консульская вализа подлежит изъятию", "консульская вализа подлежит возврату в страну отправления", "консульская вализа не может быть вскрыта", "таможенный орган самостоятельно осуществляет вскрытие"]
        },
        {
            "q": "14. Без уплаты таможенных пошлин вывозятся с таможенной территории ЕАЭС товары для личного пользования:",
            "a": ["весом не более 31 кг", "стоимостью не более 5000 евро", "стоимостью не более 5000 евро и (или) весом не более 31 кг", "независимо от их стоимости, веса и (или) количества"]
        },
        {
            "q": "15. Какой разделитель разрядов (цифр) кода используется в ГС?",
            "a": ["Пробел", "Точка", "Запятая", "Тире"]
        },
        {
            "q": "16. Какое ОПИ делится на подправила?",
            "a": ["ОПИ1", "ОПИ2", "ОПИ4", "ОПИ6"]
        },
        {
            "q": "17. Сколько дефисов стоит рядом с наименованием кода 9503 00 в ТНВЭД?",
            "a": ["один", "два", "три", "их нет"]
        },
        {
            "q": "18. Что из перечисленного не может являться объектом таможенного контроля?",
            "a": ["Документы", "Товары", "Транспортные средства международной перевозки", "Участники ВЭД"]
        },
        {
            "q": "19. ТК ЕАЭС предусматривает случаи освобождения от применения только:",
            "a": ["Таможенного осмотра", "Таможенного досмотра", "Личного таможенного досмотра", "Таможенной проверки"]
        },
        {
            "q": "20. Метод сложения – это метод №:",
            "a": ["3", "2", "5", "4"]
        }
    ]
}
...
function checkingTesting(testing) {
    ...
    async function getData() {
        ...
        for (i = 0; i < dataArray.length; i++) {
            // console.log('dataArray[i].usrName='+dataArray[i].usrName)
            // console.log('localStorage.getItem("student")='+localStorage.getItem('student'))
            // console.log('dataArray[i].date='+dataArray[i].date)
            // console.log('localStorage.getItem("dtfill")='+localStorage.getItem('student'))
            // console.log('dataArray[i].studEvent='+dataArray[i].studEvent)
            // console.log("localStorage.getItem('evtstu')="+localStorage.getItem('evtstu'))
            ...
            // console.log(dataArray[i].usrName)
            // console.log(dataArray[i].studEvent)
            // console.log(localStorage.getItem('evtstu'))
        ...
        // console.log(dataArrayUsr)
        // console.log(dataArrayAdm)
        var c = 0;
        var radioId;
        var wrongRadioId;
        var score = 0;
